# Copyright (c) 2026 Denys Nazarenko, Lviv Polytechnic National University.
"""Static substitution boxes and MDS matrices for DSTU 7624:2014 (Kalyna).

This private module stores the four forward S-boxes
:math:`\\pi_0, \\pi_1, \\pi_2, \\pi_3` and their inverses, together with the
forward and inverse MDS matrices specified in section 6 of DSTU 7624:2014.

The numerical values reproduce verbatim the constants distributed by the
authors of the standard via the official Kalyna reference implementation
maintained by Ruslan Kiianchuk, Ruslan Mordvinov and Roman Oliynykov:
https://github.com/Roman-Oliynykov/Kalyna-reference

These tables are immutable and exposed as :class:`tuple` literals so that
downstream code never accidentally mutates them.

:see: DSTU 7624:2014 Annex A.1 (S-blocks) and Annex A.2 (MDS matrix).
"""

from __future__ import annotations

from typing import Final

KALYNA_REDUCTION_POLYNOMIAL: Final[int] = 0x011D
"""Irreducible polynomial used in :math:`GF(2^8)` arithmetic.

Equal to :math:`x^{8} + x^{4} + x^{3} + x^{2} + 1`. Section 5.4 of
DSTU 7624:2014 specifies this polynomial as the modulus for byte-level
multiplications inside the linear ``ψ`` (MixColumns) transformation.
"""

KALYNA_FORWARD_SBOXES: Final[tuple[tuple[int, ...], ...]] = (
    (
        ,
    ),
    (
        ,
    ),
    (
        ,
    ),
    (
        ,
    ),
)
"""Forward S-boxes :math:`\\pi_{0..3}` of DSTU 7624:2014, section 6.2.1."""


KALYNA_INVERSE_SBOXES: Final[tuple[tuple[int, ...], ...]] = (
    (
        ,
    ),
    (
        ,
    ),
    (
        ,
    ),
    (
        ,
    ),
)
"""Inverse S-boxes :math:`\\pi^{-1}_{0..3}` of DSTU 7624:2014, section 6.2.1."""


KALYNA_MDS_FORWARD: Final[tuple[tuple[int, ...], ...]] = (
    (0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04),
    (0x04, 0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07),
    (0x07, 0x04, 0x01, 0x01, 0x05, 0x01, 0x08, 0x06),
    (0x06, 0x07, 0x04, 0x01, 0x01, 0x05, 0x01, 0x08),
    (0x08, 0x06, 0x07, 0x04, 0x01, 0x01, 0x05, 0x01),
    (0x01, 0x08, 0x06, 0x07, 0x04, 0x01, 0x01, 0x05),
    (0x05, 0x01, 0x08, 0x06, 0x07, 0x04, 0x01, 0x01),
    (0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04, 0x01),
)
"""Forward MDS matrix used by the ψ (MixColumns) transformation."""


KALYNA_MDS_INVERSE: Final[tuple[tuple[int, ...], ...]] = (
    (0xAD, 0x95, 0x76, 0xA8, 0x2F, 0x49, 0xD7, 0xCA),
    (0xCA, 0xAD, 0x95, 0x76, 0xA8, 0x2F, 0x49, 0xD7),
    (0xD7, 0xCA, 0xAD, 0x95, 0x76, 0xA8, 0x2F, 0x49),
    (0x49, 0xD7, 0xCA, 0xAD, 0x95, 0x76, 0xA8, 0x2F),
    (0x2F, 0x49, 0xD7, 0xCA, 0xAD, 0x95, 0x76, 0xA8),
    (0xA8, 0x2F, 0x49, 0xD7, 0xCA, 0xAD, 0x95, 0x76),
    (0x76, 0xA8, 0x2F, 0x49, 0xD7, 0xCA, 0xAD, 0x95),
    (0x95, 0x76, 0xA8, 0x2F, 0x49, 0xD7, 0xCA, 0xAD),
)
"""Inverse MDS matrix used by the ψ⁻¹ (InvMixColumns) transformation."""
